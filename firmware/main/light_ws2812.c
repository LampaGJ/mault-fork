...
*
* License: GNU GPL v2+ (see light_ws2812_LICENSE.txt)
*
* Vendored (not installed as a library) from
* https://github.com/cpldcpu/light_ws2812, subfolder light_ws2812_AVR -
* not registered in the Arduino Library Manager index under any name, and
* the repo root has no library.properties so `arduino-cli lib install
* --git-url` rejects it too. Guarded to compile to nothing on non-AVR
* boards (arduino-cli compiles every .c/.cpp in the sketch folder
* regardless of #include, and this file is raw AVR inline asm - see
* firmware/main/main.ino's `light` command).
*/

...
import {
  isRepackTourCompleted,
  markRepackTourCompleted,
  REPACK_TOUR_STEPS,
} from "@/features/bins/lib/repack-tour";
...
export function RepackTour({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    if (!isRepackTourCompleted()) controls.start(0);
  ...
  return (
    <>
      <Tooltip>
        ...
        />
        <TooltipContent>{t("repackTour.triggerTooltip")}</TooltipContent>
      </Tooltip>
      {Tour}
    </>
  );
